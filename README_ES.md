...
Esto es un MVP: CLI + **GUI nativa** (Windows primero, egui). La GUI de producto es **Signet only** (sin mainnet ni selector de red). `hbp-ui` localhost **no** es el producto. Disputa de producto: fee-burn t1/t2. Árbitro off. Red: Tor SOCKS5 + DHT Kademlia TCP ([docs/NETWORK.md](docs/NETWORK.md), [docs/WINDOWS.md](docs/WINDOWS.md)).
