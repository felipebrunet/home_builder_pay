...
**Quote.** **Only the mandante proposes** the FX/price. The contratista **only signs**. Main numbers stay in the **contract currency** (CLP on a CLP trato), formatted as money (`$5.000 CLP`, never `5000.00`). Sats are small print (`6.712 sats`). FX is the **quoted snapshot** (“tipo de cambio acordado: … CLP/BTC”). Pair is **contract unit per BTC** (`CLP/BTC`), never a leftover USD rate. Yadio → CoinGecko → CoinMarketCap. SATS/BTC skip FX. Both signatures required before `set_quote`. Wired as `NetMessage::Quote` over Tor.

If both sides still draft different sats (old builds / race): Pago shows **Hay dos cotizaciones distintas** with both summaries. **Usar la del otro** (replace local, keep peer sigs), **Quedarme con la mía** (ignore incoming / resend ours), **Borrar y cotizar de nuevo** (`recotizar_if_unfunded`). Same sats merge signatures. After lock + funding started, a conflicting quote is **ya está fondeando** — not a montos-distintos panic.
